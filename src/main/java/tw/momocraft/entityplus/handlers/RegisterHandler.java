...
public class RegisterHandler {
    ...
    private static void sendUtilityDepends() {
        String hookMsg = "&fHooked: ["
                + (CorePlusAPI.getDependManager().MythicMobsEnabled() ? "MythicMobs, " : "")
        ...
        try {
            CorePlusAPI.getLangManager().sendConsoleMsg(ConfigHandler.getPluginPrefix(),
                    hookMsg.substring(0, hookMsg.lastIndexOf(", ")) + "]");
        } catch (Exception ignored) {
        }

        if (CorePlusAPI.getDependManager().ResidenceEnabled()) {
            hookMsg = "&fResidence Flags: ["
                    + (CorePlusAPI.getConditionManager().isRegisteredFlag("spawnbypass") ? "spawnbypass, " : "")
                    + (CorePlusAPI.getConditionManager().isRegisteredFlag("spawnerbypass") ? "spawnerbypass, " : "")
                    + (CorePlusAPI.getConditionManager().isRegisteredFlag("damagebypass") ? "damagebypass, " : "")
            ;
            try {
                CorePlusAPI.getLangManager().sendConsoleMsg(ConfigHandler.getPluginPrefix(),
                        hookMsg.substring(0, hookMsg.lastIndexOf(", ")) + "]");
