...
public class UpdateHandler {
    ...
    public UpdateHandler() {
        this.checkUpdates(EntityPlus.getInstance().getServer().getConsoleSender(), false);
    }
    ...
    public void checkUpdates(CommandSender sender, boolean playerJoin) {
        ...
        if (this.updatesAllowed) {
            if (this.updateNeeded(sender)) {
                ...
                ServerHandler.sendMessage(sender, "&ahttps://www.spigotmc.org/resources/entityplus.70510/history");
            } else {
                if (this.betaVersion) {
