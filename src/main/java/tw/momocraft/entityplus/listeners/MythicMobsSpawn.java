...
public class MythicMobsSpawn implements Listener {
    ...
    public void onMythicMobsSpawn(MythicMobSpawnEvent e) {
        ...
        if (!getLimitAFK(e, entityType)) {
            ServerHandler.sendFeatureMessage("(MythicMobSpawn) Spawn-List", entityType, "AFK", "cancel");
            e.setCancelled();
            return;
        }
        ...
        if (!getLimit(e, entityType)) {
            ServerHandler.sendFeatureMessage("(MythicMobSpawn) Spawn-List", entityType, "Amount", "cancel");
            e.setCancelled();
            return;
        }
        ...
        if (ConfigHandler.getConfig("config.yml").getBoolean("Spawn.Enable")) {
            ...
            if (creatureConfig == null) {
                ServerHandler.sendFeatureMessage("(MythicMobSpawn) Spawn", entityType, "entityConfig = null", "return");
                return;
            }
            ...
            if (!creatureList.contains(entityType)) {
                if (!ConfigHandler.getCustomGroups(entityType, "Entities")) {
                    ServerHandler.sendFeatureMessage("(MythicMobSpawn) Spawn", entityType, "entityList not contains", "return");
                    return;
                }
            }
            ...
            if (ConfigHandler.getConfig("config.yml").getString("Spawn.List." + entityType + ".Chance") != null) {
                ...
                if (!CreatureSpawn.isChance("Spawn.List." + entityType + ".Chance")) {
                    ServerHandler.sendFeatureMessage("(MythicMobSpawn) Spawn", entityType, "!Chance", "return");
                    return;
                }

                // If the creature's spawn "biome" isn't match.
                if (!CreatureSpawn.containBiomes(loc, "Spawn." + entityType + ".Biome")) {
                    ServerHandler.sendFeatureMessage("(MythicMobSpawn) Spawn", entityType, "!Biome", "return");
                    return;
                }

                // If the creature's spawn "water" isn't match.
                if (!CreatureSpawn.isWater(loc, "Spawn." + entityType + ".Water")) {
                    ServerHandler.sendFeatureMessage("(MythicMobSpawn) Spawn", entityType, "!Water", "return");
                    return;
                }

                // If the creature's spawn "day" isn't match.
                if (!CreatureSpawn.isDay(loc, "Spawn." + entityType + ".Day")) {
                    ServerHandler.sendFeatureMessage("(MythicMobSpawn) Spawn", entityType, "!Day", "return");
                    return;
                }
                ...
                if (!LocationAPI.isBlocks(loc, "Spawn.List." + entityType + "Blocks")) {
                    ServerHandler.sendFeatureMessage("(MythicMobSpawn) Spawn", entityType, "!Blocks", "return");
                    return;
                }

                ServerHandler.sendFeatureMessage("(MythicMobSpawn) Spawn", entityType, "Final", "cancel");
                e.setCancelled();
            } else {
                ...
                while (iterator.hasNext()) {
                    group = iterator.next();
                    if (!CreatureSpawn.isChance("Spawn.List." + entityType + "." + group + ".Chance")) {
                        if (!iterator.hasNext()) {
                            ServerHandler.sendFeatureMessage("(MythicMobSpawn) Spawn", entityType, "!Chance", "return");
                            return;
                        }
                        ServerHandler.sendFeatureMessage("(MythicMobSpawn) Spawn", entityType, "!Chance", "continue", "check another group");
                        continue;
                    }

                    if (!CreatureSpawn.containBiomes(loc, "Spawn." + entityType + "." + group + ".Biome")) {
                        if (!iterator.hasNext()) {
                            ServerHandler.sendFeatureMessage("(MythicMobSpawn) Spawn", entityType, "!Biome", "return");
                            return;
                        }
                        ServerHandler.sendFeatureMessage("(MythicMobSpawn) Spawn", entityType, "!Biome", "continue", "check another group");
                        continue;
                    }

                    if (!CreatureSpawn.isWater(loc, "Spawn." + entityType + "." + group + ".Water")) {
                        if (!iterator.hasNext()) {
                            ServerHandler.sendFeatureMessage("(MythicMobSpawn) Spawn", entityType, "!Water", "return", "Water");
                            return;
                        }
                        ServerHandler.sendFeatureMessage("(MythicMobSpawn) Spawn", entityType, "!Water", "continue", "check another group");
                        continue;
                    }

                    if (!CreatureSpawn.isDay(loc, "Spawn." + entityType + "." + group + ".Day")) {
                        if (!iterator.hasNext()) {
                            ServerHandler.sendFeatureMessage("(MythicMobSpawn) Spawn", entityType, "!Day", "return");
                            return;
                        }
                        ServerHandler.sendFeatureMessage("(MythicMobSpawn) Spawn", entityType, "!Day", "continue", "check another group");
                        continue;
                    }
                    ...
                    if (!LocationAPI.checkLocation(loc, "Spawn.List." + entityType + "." + group + ".Location")) {
                        ...
                        ServerHandler.sendFeatureMessage("(MythicMobSpawn) Spawn", entityType, "!Location", "continue", "check another group");
                        continue;
                    }
                    ...
                    if (!LocationAPI.isBlocks(loc, "Spawn.List." + entityType + "." + group + "Blocks")) {
                        ...
                        ServerHandler.sendFeatureMessage("(MythicMobSpawn) Spawn", entityType, "!Blocks", "continue", "check another group");
                        continue;
                    }
                ...
                if (!ConfigHandler.getConfig("config.yml").getStringList("Spawn-Limit.Range.Ignore-Worlds").contains(e.getLocation().getWorld().getName())) {
                    if (ConfigHandler.getDepends().ResidenceEnabled()) {
                        ...
                        if (res != null) {
                            if (res.getPermissions().has("spawnlimitbypass", false)) {
                                ServerHandler.sendFeatureMessage("(MythicMobSpawn) Spawn-Limit", entityType, "ignore residence", "return", "residence has flag \"spawnlimitbypass\"");
                                return true;
                            }
                        }
                    }
                    ...
                    if (limitRangeChance != 0) {
                        ...
                        if (limitRangeChance < random) {
                            ServerHandler.sendFeatureMessage("(MythicMobSpawn) Spawn", entityType, "!getLimit - Chance", "cancel");
                            return false;
                        }
                    } else {
                        ServerHandler.sendFeatureMessage("(MythicMobSpawn) Spawn", entityType, "!getLimit - Chance = 0", "cancel");
                        return false;
                    }
                }
            }
                ...
                if (!ConfigHandler.getConfig("config.yml").getBoolean("Spawn-Limit.AFK.List-Enable") || ConfigHandler.getConfig("config.yml").getStringList("Spawn-Limit.AFK.List").contains(entityType)) {
                    if (!ConfigHandler.getConfig("config.yml").getStringList("Spawn-Limit.AFK.Ignore-Worlds").contains(e.getLocation().getWorld().getName())) {
                        ...
                        if (ConfigHandler.getConfig("config.yml").getDouble("Spawn-Limit.AFK.Max-Amount") != -1) {
                            double limitRangeAmount = ConfigHandler.getConfig("config.yml").getDouble("Spawn-Limit.Range.Max-Amount");
                            if (nearbyEntities.size() >= limitRangeAmount) {
                                ServerHandler.sendFeatureMessage("(MythicMobSpawn) Spawn", entityType, "!getLimitAFK - Max Amount", "cancel");
                                return false;
                            }
                        }
                        ...
                        if (limitAFKChance != 0) {
                            ...
                            if (limitAFKChance < random) {
                                ServerHandler.sendFeatureMessage("(MythicMobSpawn) Spawn", entityType, "!getLimitAFK - Chance", "cancel");
                                return false;
                            }
                        } else {
                            ServerHandler.sendFeatureMessage("(MythicMobSpawn) Spawn", entityType, "!getLimitAFK - Chance = 0", "cancel");
                            return false;
                        }
                    }
                }
