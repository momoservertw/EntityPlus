...
public class CreatureSpawn implements Listener {
    ...
    public void onSpawnMobs(CreatureSpawnEvent e) {
        ...
        if (ConfigHandler.getConfig("config.yml").getBoolean("Spawn.Enable")) {
            ...
            if (ConfigHandler.getConfig("config.yml").getString("Spawn.List." + entityType + ".Chance") != null) {
                ...
                List<String> worldList = ConfigHandler.getConfig("config.yml").getStringList("Spawn.List." + entityType + ".Worlds");
                ConfigurationSection worldConfig = ConfigHandler.getConfig("config.yml").getConfigurationSection("Spawn.List." + entityType + ".Worlds");
                ...
                if (!worldList.isEmpty()) {
                    ...
                    while (iterator.hasNext()) {
                        ...
                        ConfigurationSection xyzList = ConfigHandler.getConfig("config.yml").getConfigurationSection("Spawn.List." + entityType + ".Worlds." + world);
                        if (xyzList != null) {
                ...
                while (iterator.hasNext()) {
                    ...
                    List<String> worldList = ConfigHandler.getConfig("config.yml").getStringList("Spawn.List." + entityType + "." + group + ".Worlds");
                    ConfigurationSection worldConfig = ConfigHandler.getConfig("config.yml").getConfigurationSection("Spawn.List." + entityType + "." + group + ".Worlds");
                    String world;
                    if (!worldList.isEmpty()) {
                        ...
                        while (iterator2.hasNext()) {
                            ...
                            ConfigurationSection xyzList = ConfigHandler.getConfig("config.yml").getConfigurationSection("Spawn.List." + entityType + "." + group + ".Worlds." + world);
                            if (xyzList != null) {
                                for (String key : xyzList.getKeys(false)) {
     ...
     */
    static boolean getXYZFormat(String key, int xyzArgs, String[] keyValue) {
        if (xyzArgs == 1) {
            if (key.length() == 1) {
                if (key.matches("[XYZR]")) {
